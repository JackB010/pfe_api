    ...
    def post(self, request, *args, **kwargs):
        ...
        if data.is_valid():
            ...
            reply = CommentReply.objects.filter(
                Q(id=id) & Q(deleted=False) & Q(user__is_active=True)
            ).first()
        ...
        if reply:
            for i in reply.likes.all():
                if check_type(i.username) == "profile":
                    likes.append(
                        UserShortSerializer(
                            i.profile, context={"request": self.request}
                        ).data
                    )
                else:
                    likes.append(
                        PageShortSerializer(
                            i.page, context={"request": self.request}
                        ).data
                    )
...
class PostCommentUpdateAPI(generics.UpdateAPIView):
    ...
    def get_object(self, *args, **kwargs):
        id = self.kwargs.get("cid")
        queryset = CommentPost.objects.filter(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data.is_valid():
            ...
            post = Post.objects.filter(
                id=id, deleted=False, user__is_active=True
            ).first()
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        ...
        obj = get_object_or_404(
            Post, Q(deleted=False) & Q(pk=id) & Q(user__is_active=True)
        )
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if user1.id != None:
            ...
            if user == None:
                following = user1.following.filter(following__is_active=True).values(
                    "following"
                )
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if self.request.user.id != None:
            ...
            posts_m = Post.objects.filter(
                ...
                & Q(deleted=False)
                & Q(show_post_to__in=["onlyme"])
                & (Q(content__icontains=search) | Q(tags__name__icontains=search))
            )
            posts_f = Post.objects.filter(
                Q(user__in=following)
                & Q(user__is_active=True)
