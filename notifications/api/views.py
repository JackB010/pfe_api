...
def make_read_notifications(request, id=None):
    if( id==None):
        ...
        request.user.notifications.filter(Q(deleted=False) & Q(seen=False) & Q(id=id)).update(
            seen=True
        )
...
class NotificationAPI(generics.ListAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        return (
            ...
            else self.request.user.notifications.filter(Q(deleted=False) & Q(to_user__is_active=True) & Q(created_by__is_active=True))
        ).order_by("-created")
